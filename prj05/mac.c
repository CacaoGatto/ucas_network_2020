...
iface_info_t *lookup_port(u8 mac[ETH_ALEN]) {
    ...
    //fprintf(stdout, "TO: implement the lookup process here.\n");
    pthread_mutex_lock(&mac_port_map.lock);
...
void insert_mac_port(u8 mac[ETH_ALEN], iface_info_t *iface) {
    ...
    //fprintf(stdout, "TO: implement the insertion process here.\n");
    pthread_mutex_lock(&mac_port_map.lock);
    mac_port_entry_t *entry;
    u8 index = hash8(mac, ETH_ALEN);
    list_for_each_entry(entry, &mac_port_map.hash_table[index], list) {
...
int sweep_aged_mac_port_entry() {
    ...
    //fprintf(stdout, "TO: implement the sweeping process here.\n");
